...
router.get('/productos/list',async(req, res)=>{
    try {
        ...
        const [productos] = await pool.query('SELECT p.ProductoID, p.Nombre, p.Descripcion, p.Precio, c.Nombre AS Categoria FROM Productos p INNER JOIN Categorias c ON p.CategoriaID = c.CategoriaID ORDER BY p.Nombre ASC');
...
router.post('/productos/add', upload.single('Imagen'), async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            [Nombre, Descripcion, Precio, Categoria, imagenBuffer]
        );
